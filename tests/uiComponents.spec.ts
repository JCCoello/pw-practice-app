...
test.describe('Form layouts page', () => {
    ...
    test('Radio Buttons', async ({ page }) => {
        ...
        await usingTheGridForm.getByRole('radio', { name: "Option 1" }).check({ force: true }) // This is another way of writong the same code
        ...
        await usingTheGridForm.getByRole('radio', { name: "Option 2" }).check({ force: true }) // In this case we are checking that the option 2 button is checked and the Option 1 button is not
        expect(await usingTheGridForm.getByRole('radio', { name: "Option 1" }).isChecked()).toBeFalsy() // Falsy and truthy are to check if selected or not selected
